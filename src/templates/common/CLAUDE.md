...
## TypeScript Client (@polkadot-apps/contracts)

### Setup

Connect to the chain via `@polkadot-apps/chain-client` and create a `ContractManager`:
...
The `ContractManager` constructor takes three arguments:
1. `cdmJson` — the parsed `cdm.json` object (contains targets, contract ABIs, and addresses)
2. `api.contracts` — the Ink SDK from the chain API
3. `options` — optional `{ defaultSigner, signerSource }` for signing transactions

For browser apps with wallet/host signing, use `SignerManager` instead of dev signers:
...
// With arguments:
await counter.incrementN.tx(5);
```

### Dev Signers

For local development and testing, use `createDevSigner` from `@polkadot-apps/tx`:

```typescript
import { createDevSigner } from "@polkadot-apps/tx";

const signer = createDevSigner("Alice"); // "Alice" | "Bob" | "Charlie" | "Dave" | "Eve" | "Ferdie"
```

For custom mnemonics (non-dev accounts), use `seedToAccount` from `@polkadot-apps/keys`:

```typescript
import { seedToAccount } from "@polkadot-apps/keys";

const account = seedToAccount(mnemonic, "//0");
// account.signer — PolkadotSigner
// account.ss58Address — SS58 encoded address
```

### Cleanup

```typescript
api.destroy(); // closes all chain connections
```
