...
`cdm install` (alias `cdm i`) fetches published contract ABIs for Rust imports and product-sdk TypeScript usage:
...
2. Fetches ABI from the configured Bulletin IPFS gateway
3. Saves to `~/.cdm/<targetHash>/contracts/<library>/<version>/abi.json`
4. Updates `cdm.json` with contract address, ABI, version, and CID
5. Generates `.cdm/contracts.d.ts` TypeScript type augmentations for `@parity/product-sdk-contracts`
...
- **TypeScript apps:** `ContractManager.getContract("@polkadot/reputation")` from `@parity/product-sdk-contracts`

Do not use `@dotdm/cdm` in frontend/app code. CDM is the CLI/install/deploy layer; product-sdk is the app/runtime contract layer.
...
## TypeScript Apps (product-sdk contracts)

Frontend and Triangle/Polkadot Desktop apps should use product-sdk contract tooling. The contracts package reads the `cdm.json` generated by `cdm install`, resolves addresses and ABIs, and exposes typed `.query()`, `.tx()`, and `.prepare()` methods.
...
### Setup with Product Account signing
...
const contract = contracts.getContract("@org/contract");
const { value } = await contract.someRead.query();
await contract.someWrite.tx("arg");
```

### Querying, transactions, and batching

Every contract method exposes:

- `.query(...args, opts?)` — read-only dry-run; no signature
- `.tx(...args, opts?)` — signs, submits, and watches a `Revive.call`
- `.prepare(...args, opts?)` — creates a batchable call for `batchSubmitAndWatch`

```typescript
import { batchSubmitAndWatch } from "@parity/product-sdk-tx";

const a = contract.firstAction.prepare();
const b = contract.secondAction.prepare(123n);

await batchSubmitAndWatch([a, b], chain.raw.assetHub, product.value.getSigner());
```

### Generated Types (.cdm/contracts.d.ts)

`cdm install` generates module augmentation for `@parity/product-sdk-contracts`:
...
With `.cdm/**/*` included in `tsconfig.json`, `contracts.getContract("@example/counter")` returns a fully typed contract handle.

**ABI -> TypeScript type mapping:** `uint8/16/32` -> `number`, `uint64+` -> `bigint`, `address` -> `HexString`, `string` -> `string`, `bool` -> `boolean`, `bytes` -> `HexString`, `bytesN` -> `SizedHex<N>`, `tuple` -> `{ field: type }`.

### Common mistakes

- **Using `@dotdm/cdm` in a frontend:** use `@parity/product-sdk-contracts` instead.
- **Using raw PAPI clients in Triangle apps:** prefer `getChainAPI("paseo")` and host-backed product-sdk flows.
- **Forgetting account mapping:** call `ensureContractAccountMapped(...)` before the first `.tx()` for a Product Account.
- **Using the Bulletin RPC as an IPFS gateway:** `cdm.json.targets[*].bulletin` is an HTTP gateway such as `https://paseo-bulletin-next-ipfs.polkadot.io/ipfs`.

Runtime: browser app inside the Polkadot Desktop/Triangle host. Package manager: pnpm or bun.
