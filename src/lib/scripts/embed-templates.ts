...
// Collect common files shared across all templates
const COMMON_DIR = join(TEMPLATES_DIR, "common");
const commonFiles = readdirSync(COMMON_DIR, { withFileTypes: true }).length
    ? collectFiles(COMMON_DIR, COMMON_DIR)
    : {};

// Discover all template directories (skip "common" — it's merged into each template)
const templateDirs = readdirSync(TEMPLATES_DIR, { withFileTypes: true }).filter(
    (d) => d.isDirectory() && d.name !== "common",
);

let output = "// Auto-generated by scripts/embed-templates.ts — do not edit\n\n";

const names: string[] = [];

for (const dir of templateDirs) {
    const templatePath = join(TEMPLATES_DIR, dir.name);
    const files = { ...commonFiles, ...collectFiles(templatePath, templatePath) };
