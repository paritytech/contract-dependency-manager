...
- **③ SUBMIT** — per chunk: first upload metadata blobs to Bulletin (sequential per Bulletin's nonce ordering, returns CIDs), then a single `Utility.batch_all` atomically issues `Revive.instantiate_with_code` (the version's implementation contract) and `Registry.publish(name, versionKey, addr, cid)`. On a name's first publish the registry also CREATE2-instantiates the name's proxy inside the same call. A version with a matching initialization — addressed by (contract, version): Rust `<crate>/initializations/<version>.rs` (the crate names the contract), Solidity `initializations/<Contract>/<version>.sol` (the directory names the contract) — additionally instantiates the initialization contract in the same batch and registers via `Registry.publishWithInit(..., initAddr)` — the registry delegate-calls the initialization against the proxy's storage through the admin-only `callCode` meta op, so the publish and its one-shot initialization land or roll back together. Atomic within a chunk; chunks across are non-atomic by design.
